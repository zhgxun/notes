 ...
 * 压缩列表
 * 使用压缩列表的条件
 * 列表中保存到单个数据小于64字节, 列表中保存的数据个数小于512个
 * <p>
 * 双向循环链表
 * <p>
 * 字典
 * 其实就是哈希
 * 也是压缩列表和散列表一起实现
 * 压缩列表的条件同样是字典中保存的键和值大小小于64字节, 字典中保存的数据个数小于512个
 * <p>
 * Redis使用 MurmurHash2 这种运行速度快, 随机性好的哈希算法作为哈希函数, 对于哈希冲突, 使用链表法解决, 还支持动态缩容扩容
 * <p>
 * 集合
 * 有序数组和散列表
 * <p>
 ...
 * <p>
 * 数据结构持久化
 * 数据结构的持久化问题, 也是对象的持久化问题, 可以笼统的理解为存储到磁盘
 * 第一种思路
 * 清除原有的数据结构, 只将数据存储到磁盘中, 当我们需要从磁盘还原数据到内存的时候, 再重新将数据组织成原来的数据结构, 实际上, Redis
 * 采用的就是这种持久化思路
 * 这种方式有一定的弊端, 就是数据从磁盘还原到内存的过程中, 会耗用比较多的时间, 比如我们现在要将散列表中的数据存储到磁盘, 当我们从磁盘中
 * 取出数据重新构建散列表的时候, 需要重新计算每个数据的哈希值, 如果磁盘中数据量很大, 重构数据结构的耗时就需要注意了
 * <p>
 * 第二种思路
 * 保留原来的存储方式, 将数据按照原有的格式存储到磁盘中, 拿散列表来举例就是把散列表的大小, 每个数据被散列到的槽号的编号等信息都保存在磁盘中
 * 有了这些信息, 从磁盘中将数据还原到内存中的时候, 就避免重新计算哈希值
 */
