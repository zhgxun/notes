 ...
 * <p>
 * 19. 进程间通信方式
 * 无名管道(pipe), 数据在父子进程间单向流动
 * 高级管道(popen), 将另一个程序当做一个新的进程在当前程序中启动, 则它算是当前程序的子进程, 这种方式成为高级管道方式
 * 有名管道(named pipe), 半双工的通信方式, 允许无亲缘关系进程间通信
 * 消息队列(message queue), 存放在内核中并由消息队列标识符标识, 消息队列克服了信号传递信息少, 管道只能承载无格式字节流一级缓冲区大小受限的缺点
 * 信号量(semophore), 信号量是一个计数器, 可以控制多个进程对共享资源的访问, 常作为一种锁机制, 防止某进程正在访问共享资源时, 其它进程也访问该资源
 * 信号(sinal), 信号是一种比较复杂的通信方式, 用于通知接收进程的某个事件已经发生
 * 共享内存(shared memory), 共享内存是映射一段能被其他进程所访问的内存, 这段共享内存由一个进程创建, 但多个进程都可以访问, 共享内存是最快的 IPC 方式
 * 它是针对其它进程间通信方式运行效率低而专门设计的, 往往与其它通信机制配合使用
 * 套接字(socket), 与其它通信机制不同的是可用于不同机器进程间通信
 */
